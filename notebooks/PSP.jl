...
using NeumannKelvin, JSON, StaticArrays, LinearAlgebra, Plots, PlotlyBase, PlutoUI
...
function psShape(ship_info::Dict{String, Any})
	...
	# Calculate total width of the PS        
		...
		for θ in range(0, stop=π/2, length=6)
			x = bow_length - bow_radius + bow_radius/2 * cos(θ)
...
function createPanel(vertices::Array{Array{Float64, 1}, 1}, centre::Array{Float64, 1}, normal::Array{Float64, 1}, faces::Array{Int64, 1}; demihull=false)
	if centre[3] > 0 || normal == [0.0, 0.0, 1.0] || length(faces) < 4
		return nothing
	end
	if demihull && centre[2] == 0 && normal == [0.0, 0.1, 0.0]
		return nothing
	end
...
function importMesh(filename::String)
	...
	demi=false
	if occursin("half", filename)
		demi = true
	end
	panels=[createPanel(vertices, centres[i], normals[i], faces[i]; demihull=demi) for i in eachindex(faces)]
...
function solve_sources(panels; demi=false)
	if demi
		ps = (ϕ=∫surface_S₂,Fn=0.4)
	else
...
begin
	panels_half, _ = importMesh(joinpath(data_folder,"PS_hull_0310_14-41_half.json"))

	ps_half = (ϕ=∫surface_S₂,Fn=0.4)
	A = influence(panels;ps...)
	b = first.(panels.n)
	added_mass_half = added_mass(panels_half; ps_half)
...
q_full, ps_full = solve_sources(panels_full)
added_mass_full = added_mass(panels_full; ps_full)
end

# ╔═╡ 9d9ad896-0388-4802-bb0e-bc7f62db127f

Plots.contourf(-200:5:200,-200:5:200,(x,y)->2ζ(x,y,q_half,panels_half;ps_half...),
	c=:balance,aspect_ratio=:equal,clims=(-0.2,0.2));Plots.plot!(shape_full, c=:black,legend=nothing)


# ╔═╡ 2e6e7dd8-54cd-419a-8342-1c8e5ec8b972
...
manifest_format = "2.0"
project_hash = "9a63813bc4a8c41cdb48792e095f72d5caf56edf"
