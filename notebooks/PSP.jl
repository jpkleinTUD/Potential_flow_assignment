...
using NeumannKelvin, JSON, StaticArrays, LinearAlgebra, Plots

# ╔═╡ bc054f16-5c7a-4b8d-b9af-abe0e1965573
using NeumannKelvin:kelvin,wavelike,nearfield

# ╔═╡ acf60230-1208-42c8-afc8-afd3ba267dd0
include(joinpath(@__DIR__, "..", "src", "PSParametric.jl"))
...
function psShape(ship_info::Dict{String, Any})
	...
	# Calculate total width of the PS
        total_width = slot_width + 2 * bow_width
        
        # Points for the rectangular body
        rect_points = [
            (0.0, -bow_width),                  # Bottom left of rectangle
            (length, -bow_width),               # Bottom right of rectangle
            (length, -bow_width + total_width), # Top right of rectangle
            (0.0, -bow_width + total_width)     # Top left of rectangle
        ]
        ...
        push!(bow1_points, (length, -bow_width))
        
        # Add rounded corner points
        for θ in range(-π/2, stop=0, length=10)
            x = length + bow_length - bow_radius + bow_radius * cos(θ)
            y = -bow_width + bow_radius + bow_radius * sin(θ)
            push!(bow1_points, (x, y))
        end
        
        # Points for bow 2 (top bow)
        bow2_points = []
        
        # Start from the top right of rectangle
        push!(bow2_points, (length, -bow_width + total_width))
        
        # Add rounded corner points
        for θ in range(0, stop=π/2, length=10)
            x = length + bow_length - bow_radius + bow_radius * cos(θ)
            y = -bow_width + total_width - bow_radius + bow_radius * sin(θ)
            push!(bow2_points, (x, y))
        end
        ...
        all_points = vcat(
            rect_points,
            bow1_points,
            [(length + bow_length, -bow_width + bow_radius)],  # Straight part of bow 1
            [(length + bow_length, -bow_width + total_width - bow_radius)],  # Straight part of bow 2
...
# ╔═╡ a20c1b4c-f8d2-4457-b048-7869fff8841a
begin 
...
begin
	...
	q = influence(panels;ps...)\first.(panels.n) # solve for densities
end;

# ╔═╡ 9998b3e0-a799-42a5-889a-91908d1268dd
begin
Plots.contourf(-400:5:200,-200:5:200,(x,y)->2ζ(x,y,q,panels;ps...),
