...
begin
	using StaticArrays,TypedTables,Plots
	using ForwardDiff: derivative,gradient
	using NeumannKelvin: param_props,∫G
...
My Julia pacakge `NeumannKelvin.jl` implements 3D panel potential using Gauss quadratures. Check the documentation for `∫G`, or [check the code](https://github.com/weymouth/NeumannKelvin.jl/blob/dcc8611a04beade8923ca9b45765da577cf2f5ad/src/panel_method.jl#L41). 
...
# ╔═╡ 4cb8943f-2724-4b47-b0e2-3a04cbcb8a85


# ╔═╡ 0576ce9d-dd33-4b20-915a-24421037d7c0
function cylinder(h; L=1, R=1)
	S(θ, z) = R .* SA[cos(θ), sin(θ), z]  # Cylinder lateral surface equation
	S_bottom(θ, r) = SA[r * cos(θ), r * sin(θ), -L/2]
	S_top(θ, r) = SA[r * cos(θ), r * sin(θ), L/2]

	dr = (h) / R
	dθ = π/round(π * R / 0.5h)  # Step size for azimuthal angle
	dz = 0.5L*h  # Step size for height

	lateral_panels = param_props.(S, (0:dθ:2π)', -L/2:dz:L/2, dθ, dz) |> Table

	bottom_and_top_panels = mapreduce(vcat, 0.5dr:dr:R) do r
	    dθr = (2π * (1 - (r/R))) * h
	    bottom_panels = param_props.(S_bottom, (0:dθr:2π)', r, dθr, dr) |> Table
	    top_panels = param_props.(S_top, (0:dθr:2π)', r, dθr, dr) |> Table
	    vcat(bottom_panels, top_panels)  
	end

	return vcat(bottom_and_top_panels, lateral_panels)
	#return bottom_and_top_panels
...
h = 0.5; panels = cylinder(h); display(panels)
...
begin 
	...
	N=length(panels)
	scatter(x,y,z,legend=false,title="Sphere represented with $N panels")
end
...
begin
	L=1
	A_error = sum(panels.dA)/(2π*(L+1))-1
	print("Total surface area error: $(round(100A_error,digits=1))% \r\n")
	print("Total surface area: $(round(sum(panels.dA), digits=1))m^2 \r\n")
	print("Analytical surface area:$(round(2π*(L+1), digits=1))m^2")
end
...
begin
	filtered_panels = filter(p -> p.x[3] == 0, panels)
	display(filtered_panels.x)
	N_filtered = length(filtered_panels)
	# Velocity magnitude |u| on panel centroids
	abs_u(q,filtered_panels) = map(x->hypot(SA[-1,0,0]+∇Φ(x,q,filtered_panels)...),filtered_panels.x)
	
	# Get the angle and velocity
	function alpha_velocity(filtered_panels)
		A,b = ∂ₙϕ.(filtered_panels,filtered_panels'),first.(filtered_panels.n)
		# cos(α) = x/R = b
		return acos.(b),abs_u(A\b,filtered_panels)
	end
	
	plot(0:0.01:π,x->(2)sin(x),label="exact",xlabel="α",ylabel="uₐ")
	α,uₐ = alpha_velocity(filtered_panels)
	scatter!(α,uₐ,label="$N_filtered panels")
end
