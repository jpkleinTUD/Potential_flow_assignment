...
# ╔═╡ 648e86a3-3ba9-4c74-a659-3590ddc10ffe
...
begin
	...
	function wigley_hull(hx,hz;D=1/8)
		...
		η_r_1(ξ,ζ) = -((1-ξ^2)*(1-ζ^2)-offset)          # parabolic width equation
	    S_r_1(ξ,ζ) = SA[0.5L*ξ,0.5B*η_r_1(ξ,ζ),-D*ζ]    # scaled 3D surface
		η_l_2(ξ,ζ) = (1-ξ^2)*(1-ζ^2)-offset             # parabolic width equation
	    S_l_2(ξ,ζ) = SA[0.5L*ξ,0.5B*η_l_2(ξ,ζ),-D*ζ]    # scaled 3D surface
		η_r_2(ξ,ζ) = -((1-ξ^2)*(1-ζ^2)+offset)          # parabolic width equation
	    S_r_2(ξ,ζ) = SA[0.5L*ξ,0.5B*η_r_2(ξ,ζ),-D*ζ]    # scaled 3D surface
		
	    dξ_in = 1/round(0.5L/hx); ξ_in = (L/2-slot_len-0.5dξ_in):dξ_in:1
		...
		dω = 4/round(offset/hz); ω = 0.5dω:dω:1 
		...
		S_bap(ω,ζ) = SA[-(L/2),η_bp(ω),-D*ζ]    # bottom plate function
		S_bop(ω,ξ) = SA[0.5L*ξ,η_bp(ω),-D] 		# back plate function
...
# Ship flow and force predictions

For the last part of the potential flow section, we'll apply our linear solver to the flow around a ship. 
...
manifest_format = "2.0"
project_hash = "b6f4f75452217577016cd31287785dccd79bd14b"
